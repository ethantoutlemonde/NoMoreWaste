...
export default function FoodCollectionParticipants({foodCollection, fetchData}) {
    ...
    const handleDelete = async () => {
        ...
        try {
            await Promise.all(selection.map(itemId => axiosClient.delete(`/api/foodCollection/${foodCollection.id}/participants`, {params: {volunteer_id: itemId}})));
    ...
    const handleAdd = () => {
        ...
        axiosClient.post(`/api/foodCollection/${foodCollection.id}/participants`, {volunteer_id: selectedVolunteer})
        .then(response => {
    ...
    return (
        ...
        <h2 className="text-xl font-medium mb-4">List of all participants</h2>
                <DataGrid 
                    rows={foodCollection?.participants} 
                    columns={[
                ...
                <select name="user_id" value={selectedVolunteer} onChange={(e) => setSelectedVolunteer(e.target.value)} className="bg-slate-100 rounded p-2 mr-2">
                        ...
                        {volunteers.map((volunteer, index) => (
                            <option key={index} value={volunteer.id}>{volunteer.first_name} {volunteer.last_name}</option>
                        ))}
