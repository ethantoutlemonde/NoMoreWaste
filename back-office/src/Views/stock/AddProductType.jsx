    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.status === 201) { 
                ...
                console.error('Failed to add product type');
            }
        } catch (error) {
            if (error.response && error.response.data && error.response.data.error) {
                setError(t(error.response.data.error));
            } else {
                setError(t("An error occurred while adding the product type."));
            }
    ...
    return (
        <div>
            ...
            <form onSubmit={handleSubmit} className="max-w-md mx-auto">
                ...
                <button 
                    type="submit" 
                    className="text-white bg-purple-600 hover:bg-purple-500 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    {t('Add')}
                </button>
            </form>
        </div>
